...
class StudentModel(object):
    ...
    def __init__(self, is_training, config):
        ...
        output = tf.reshape(tf.concat(outputs, axis=1), [-1, final_hidden_size])
...
def read_data_from_csv_file(fileName):
    ...
    while(index < len(rows)-1):
        # number of problem a student answered
        problems_num = int(rows[index][0])

        # the maximum question_id in the list
        tmp_max_skill = max(map(int, rows[index+1]))

        # update the maximum question_id
        if(tmp_max_skill > max_skill_num):
            max_skill_num = tmp_max_skill

        # keep the student with only one records
        if(problems_num <= 2):
            index += 3

        # construct a student tuples, it is in form of
        # ([number of problem answered], [question id answered], [correct map])
        else:
            # find the maximum number of question attempted by a student
